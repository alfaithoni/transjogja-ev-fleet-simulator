...
(Table~\ref{tab:regime} and Fig.~\ref{fig:regime}).
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\columnwidth]{fig1_regime_shift.pdf}
\caption{Degraded routes per day as a function of connector count at a
single station. The dashed line marks the regime boundary between 19
and 18 connectors, where removing one connector nearly doubles daily
route degradation (5.52 to 8.85).}
\label{fig:regime}
\end{figure}
...
Table~\ref{tab:main} and Fig.~\ref{fig:tradeoff}--\ref{fig:headway}
compare the two intervention families against the
...
\caption{Trade-off between mean realised headway and degraded routes per
day for each scenario. Infrastructure scenarios (blue squares) improve
headway; algorithm scenarios (red triangles) reduce degradation. Only
the combined S33 (purple diamond) achieves zero degradation.}
\label{fig:tradeoff}
\end{figure}

\begin{figure*}[htbp]
\centering
\includegraphics[width=\textwidth]{fig3_headway_comparison.pdf}
\caption{Mean realised headway (with $\pm 1\sigma$ error bars) and
degraded-route rate for each intervention scenario. The red dotted line
marks the tightest agency target (20 min). Degraded-route values are
annotated above each bar. Infrastructure scenarios (blue) achieve the
best headway; algorithm scenarios (red) achieve the lowest degradation;
the combined scenario S33 (purple) achieves zero degradation.}
\label{fig:headway}
\end{figure*}
